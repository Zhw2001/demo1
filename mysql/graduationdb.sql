...
CREATE TABLE `admin_authority` (
  `authority_id` bigint NOT NULL AUTO_INCREMENT COMMENT 'id',
  ...
  `authority_sort` int DEFAULT '-1' COMMENT '权限树的排序',
  ...
  `auth_create_date` datetime DEFAULT NULL COMMENT '创建时间',
...
CREATE TABLE `admin_role` (
  ...
  `role_create_date` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`role_id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb3 COLLATE=utf8_unicode_ci ROW_FORMAT=DYNAMIC;
...
) ENGINE=InnoDB AUTO_INCREMENT=56 DEFAULT CHARSET=utf8mb3 COLLATE=utf8_unicode_ci ROW_FORMAT=DYNAMIC;
...
CREATE TABLE `course` (
  ...
  `degree` varchar(255) DEFAULT NULL COMMENT '总课程目标达成度',
...
CREATE TABLE `courseaudit` (
  ...
  `objective_num` int DEFAULT NULL COMMENT '课程目标号',
...
CREATE TABLE `courseinfo` (
  ...
  `course_type` int DEFAULT '0' COMMENT '课程的类型：0-理论课，1-实验课，2-课程设计，3-毕业设计',
  `test_type` int DEFAULT NULL COMMENT '考核类型：0-考试，1-考察',
  `standard_num` int DEFAULT NULL COMMENT '考核标准数量',
  `standard_names` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '考核标准名称明细，JSON一维String数组',
  `question_num` int DEFAULT NULL COMMENT '考试大题数量',
  `question_names` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '考试大题名称明细，JSON一维String数组',
  `goal_num` int DEFAULT NULL COMMENT '教学目标数量',
  `score_distribution` varchar(255) DEFAULT NULL COMMENT '分数分布，JSON二维float数组，size=goal_num*(standard_num+question_num)',
  PRIMARY KEY (`info_cNum`),
  KEY `info_cId` (`info_cId`)
...
CREATE TABLE `evalinfo` (
  ...
  PRIMARY KEY (`cid`),
  CONSTRAINT `fk_evalinfo_courseinfo` FOREIGN KEY (`cid`) REFERENCES `courseinfo` (`info_cId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用来保存上次客户在网页段提交的课程审核信息，例如内容审核表中的介绍等信息、课程目标描述信息、分值分配信息等。';
...
CREATE TABLE `finalitem` (
  ...
  `courseAudit_id` bigint NOT NULL,
  PRIMARY KEY (`question_num`,`courseAudit_id`)
...
CREATE TABLE `usualitem` (
  ...
  `courseAudit_id` bigint NOT NULL,
  PRIMARY KEY (`courseAudit_id`,`part`)
